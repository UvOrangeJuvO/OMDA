...
> 更新：2026-08-19（G0）。优先级含义：P0 数据损坏/安全/核心语义错误，绝对阻断；P1 可复现功能错误/架构违约，阻断；P2 可维护性/小风险，视数量与影响而定（OPH §8.3）。
...
| R-002 | P0 | 凭据、cookie 或浏览器 profile 进入 Git/模型上下文 | .gitignore 复核、secret 扫描、字段化脱敏日志；prompt 不含敏感材料 | G1 T1.1、G5 T5.3 | OPEN |
